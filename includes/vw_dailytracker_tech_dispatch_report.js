...
module.exports = (companyId, projectId, rawDataset) =>
  ...
  SELECT TRIM(te.name)                                                                                  AS \`Name\`
       ...
       , DATE(\`pph-central.settings.fn_convert_utc_localtz\`(gpi.started_on, ${companyId}))            AS \`Timesheet Activity Date\`
       , j.job_number                                                                                   AS \`Job Number\`
       , TIME(\`pph-central.settings.fn_convert_utc_localtz\`(gpi.started_on, ${companyId}))            AS \`Start Time\`
       , CASE 
           WHEN gpi.paid_time_type = 'Regular' THEN ROUND(CAST(gpi.paid_duration_hours AS FLOAT64), 2)
           ELSE ROUND(0, 2)
         END                                                                                            AS \`Regular Time\`
       , TIME(\`pph-central.settings.fn_convert_utc_localtz\`(gpi.ended_on, ${companyId}))              AS \`End Time\`
