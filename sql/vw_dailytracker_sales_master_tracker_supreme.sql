SELECT j.job_number                                                                                   AS `Job #`
     ...
     , DATE(`pph-central.settings.fn_convert_utc_localtz`(j.completed_on,22))                         AS `Completion Date`
     , p.number                                                                                       AS `Project`
     , COALESCE(it.total_invoice,0)                                                                   AS `Job Subtotal`
     , j.customer_id                                                                                  AS `Customer Id`
     , TRIM(c.name)                                                                                   AS `Customer Name`
     , IFNULL(st.assigned_technicians, '')                                                            AS `Assigned Technicians`
     , TRIM(tb.name)                                                                                  AS `Sold By`
     , ROUND(ts.total_paid_time,2)                                                                    AS `Total Technicias Paid Time`
     , TRIM(st.primary_technician)                                                                    AS `Primary Technician`
     , j.job_status                                                                                   AS `Status`
     , DATE(`pph-central.settings.fn_convert_utc_localtz`(ap.start_appointment,22))                   AS `Scheduled Date`
     , ap.total_appointments                                                                          AS `Total Appointments`
     , DATE(`pph-central.settings.fn_convert_utc_localtz`(ap.first_appointment,22))                   AS `First Appt Date`
     , DATE(`pph-central.settings.fn_convert_utc_localtz`(ap.first_appointment,22))                   AS `Last Appt Date`
     , DATE(`pph-central.settings.fn_convert_utc_localtz`(ap.next_appointment,22))                    AS `Next Appt Start date`
     ...
     , j.id                                                                                           AS `Invoice #`
  FROM `shape-sst-22.servicetitan_shape_sst_22.job`                                                   j
  LEFT JOIN `shape-sst-22.silver.vw_job_type`                                                         jt
  ...
  LEFT JOIN
        (
         ...
         SELECT job_generated_lead_source_job_id                                                      AS job_generated_lead_source_job_id
           ...
           FROM `shape-sst-22.servicetitan_shape_sst_22.job`                                          jobs_lead
         ...
         jobs_lead_created_by AS (
           SELECT jlc.job_generated_lead_source_job_id                                                AS job_generated_lead_source_job_id
             ...
             LEFT JOIN `shape-sst-22.servicetitan_shape_sst_22.job`                                   jobs_individual
               ON jobs_individual.id                                                                  = lead_created_id
             LEFT JOIN `shape-sst-22.bronze.technician`                                               tl
  ...
  LEFT JOIN `shape-sst-22.servicetitan_shape_sst_22.business_unit`                                    bu
    ON bu.id                                                                                          = j.business_unit_id
  LEFT JOIN `shape-sst-22.servicetitan_shape_sst_22.project`                                          p
    ON p.id                                                                                           = j.project_id
  LEFT JOIN `shape-sst-22.bronze.technician`                                                          t
  ...
  LEFT JOIN
       (
        SELECT invoice_lead.job_id                                                                    AS job_id
          ...
          FROM `shape-sst-22.servicetitan_shape_sst_22.invoice`                                       invoice_lead
  ...
  LEFT JOIN
       (
        SELECT j_lead.job_generated_lead_source_job_id                                                AS parent_job_id
          ...
          FROM `shape-sst-22.servicetitan_shape_sst_22.estimate`                                      estimate
          JOIN `shape-sst-22.servicetitan_shape_sst_22.job`                                           j_lead
  ...
  LEFT JOIN
       (
        SELECT invoice_total.job_id                                                                   AS job_id
          ...
          FROM `shape-sst-22.servicetitan_shape_sst_22.invoice`                                       invoice_total
         GROUP BY invoice_total.job_id
       )                                                                                              it
    ON it.job_id                                                                                      = j.id
  LEFT JOIN `shape-sst-22.servicetitan_shape_sst_22.customer`                                         c
    ON c.id                                                                                           = j.customer_id
  LEFT JOIN `shape-sst-22.bronze.technician`                                                          tb
    ON tb.id                                                                                          = j.sold_by_id
  LEFT JOIN `shape-sst-22.dashboards.vw_jobs_technicians`                                             st
    ON st.job_id                                                                                      = j.id    
  LEFT JOIN
       (
  ...
  LEFT JOIN
       (
        SELECT estimates.job_id                                                                       AS estimates_job_id
          ...
          FROM `shape-sst-22.servicetitan_shape_sst_22.estimate`                                      estimates
          LEFT JOIN `shape-sst-22.servicetitan_shape_sst_22.estimate_item`                            estimate_items
  ...
  LEFT JOIN `shape-sst-22.bronze.campaign`                                                            ca
  ...
  LEFT JOIN
       (
        SELECT jobs_timesheets.job_id                                                                 AS job_id
          ...
          FROM `shape-sst-22.bronze.timesheet`                                                        jobs_timesheets
  ...
  LEFT JOIN
       (
        SELECT job_id
          ...
          FROM `shape-sst-22.silver.vw_purchase_order`                                                purchase_orders
  ...
  LEFT JOIN
       (
        SELECT job_id
          ...
          FROM `shape-sst-22.silver.vw_return`                                                        returns
         GROUP BY                                                                                     returns.job_id
       )                                                                                              re
    ON re.job_id                                                                                      = j.id
  LEFT JOIN `shape-sst-22.dashboards.vw_pulse_wip_appointments`                                       ap
    ON ap.job_id                                                                                      = j.id
 WHERE (EXTRACT(YEAR FROM `pph-central.settings.fn_convert_utc_localtz`(ap.first_appointment,22))     >= 2025
        OR
        EXTRACT(YEAR FROM `pph-central.settings.fn_convert_utc_localtz`(ap.start_appointment,22))     >= 2025
        OR
        EXTRACT(YEAR FROM `pph-central.settings.fn_convert_utc_localtz`(ap.next_appointment,22))      >= 2025
       )
