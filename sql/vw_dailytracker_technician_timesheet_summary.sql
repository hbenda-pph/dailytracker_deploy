-- CREATE OR REPLACE VIEW `shape-mhs-1.dashboards.vw_dailytracker_technician_timesheet_summary` AS
WITH raw_data AS (
    SELECT * FROM `shape-mhs-1.dashboards.vw_dailytracker_timestamp_base` 
  ),
  job_records AS (
  ...
  all_timesheet_codes AS (
    ...
    SELECT DISTINCT code, type
      FROM `shape-mhs-1.servicetitan_shape_mhs_1.timesheet_code`
     WHERE active = TRUE
...
SELECT ct.emp_id                                                                                                   AS `Employee Id`
  ...
  LEFT JOIN `shape-mhs-1.bronze.payroll_settings` ps ON ct.emp_id = ps.employee_id
