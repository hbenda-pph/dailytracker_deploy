...
WITH
  ...
  times_stg AS (
    SELECT
      db.*,
      DATETIME(db.started_on, 'America/New_York')
        AS started_on_local,  -- Replace timezone name if 1 does not mean EST
      DATETIME(db.ended_on, 'America/New_York') AS ended_on_local
