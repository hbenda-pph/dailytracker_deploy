...
SELECT TRIM(te.name)                                                                          AS name
  ...
  LEFT JOIN `shape-mhs-1.silver.vw_dailytracker_technicians_employees`                        te
...
SELECT name                                                                                   AS `Name`
     ...
     , DATE(`pph-central.settings.fn_universalts_localtz`(started_on,1))                      AS `Timesheet Activity Date`          
     , job_number                                                                             AS `Job Number`
     , TIME(`pph-central.settings.fn_universalts_localtz`(started_on,1)) AS `Start Time`
     , CASE 
        WHEN paid_time_type = 'Regular' THEN ROUND(CAST(paid_duration_hours AS FLOAT64), 2)
        ELSE ROUND(0, 2)
       END                                                                                    AS `Regular Time`
     , TIME(`pph-central.settings.fn_universalts_localtz`(ended_on,1))                        AS `End Time`
     ...
     , CASE 
       ...
       END                                                                                    AS `Overtime`
 FROM dispatch_base
--WHERE EXTRACT(YEAR FROM `pph-central.settings.fn_universalts_localtz`(`date`,1))             = 2026
