-- CREATE OR REPLACE VIEW `shape-mhs-1.silver.vw_dailytracker_tech_dispatch_report` AS
WITH
  dispatch_base AS (
    SELECT
      gpi.employee_id,
  ...
  -- Materialize the UDF calculations first to avoid correlation issues in the final joins
  times_stg AS (
    SELECT
      db.*,
      `pph-central.settings.fn_convert_utc_localtz`(db.started_on, 1)
        AS started_on_local,
      `pph-central.settings.fn_convert_utc_localtz`(db.ended_on, 1) AS ended_on_local
    FROM dispatch_base db
  ),
  -- Alternatively, if the UDF is strictly necessary, ensure it is calculated in a "flattened" way
  -- dispatch_summary step remains the same to organize logic
  dispatch_summary AS (
    SELECT
      ts.employee_id,
      ts.job_id,
      ...
      CASE
        WHEN ts.paid_time_type = 'Regular'
          THEN ROUND(CAST(ts.paid_duration_hours AS FLOAT64), 2)
        ELSE ROUND(0, 2)
        END
        AS `Regular Time`,
      CASE
        WHEN ts.paid_time_type = 'Overtime'
          THEN ROUND(CAST(ts.paid_duration_hours AS FLOAT64), 2)
        ELSE ROUND(0, 2)
        END
        AS `Overtime`,
...
SELECT
  ...
  CASE
    WHEN ds.paid_time_type = 'Regular'
      THEN ROUND(CAST(ps.hourly_rate AS FLOAT64), 2)
    WHEN ds.paid_time_type = 'Overtime'
      THEN ROUND(CAST(ps.hourly_rate AS FLOAT64) * 1.5, 2)
    WHEN ds.paid_time_type = 'DoubleOvertime'
      THEN ROUND(CAST(ps.hourly_rate AS FLOAT64) * 2, 2)
    WHEN ds.paid_time_type = 'PaidTimeOff'
      THEN ROUND(CAST(ps.hourly_rate AS FLOAT64) * 0, 2)
